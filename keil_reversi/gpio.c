...
void GPIO_marcar_entrada(uint32_t bit_inicial, uint32_t num_bits){
    uint32_t izq = (IOPIN >> (bit_inicial + num_bits - 1)) << (bit_inicial + num_bits - 1); // nos quedamos con los bits de la izq a los que queremos modificar
    uint32_t der = (IOPIN << (MAX - bit_inicial)) >> (MAX - bit_inicial); // nos quedamos con los bits de la drcha a los que queremos modificar
...
void GPIO_marcar_salida(uint32_t bit_inicial, uint32_t num_bits){
    ...
    uint32_t i;
    for (i = inicial; i < final; i <<= 1){     // con el 1 desplazamos 1 bit hacia la izq y con el = actualizamos el valor de j
        IODIR = i;          // liampiamos los bits que vamos a sobreescribir 
    }
}
