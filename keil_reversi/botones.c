...
// limpiar el registro EXTINT, después comprobamos en el if con una AND si EINT1 está pulsado, 
// si la and se cumple sería porque a pesar de limpiar la interrupción el botón seguía pulsado, 
// pero si no se cumple la AND al dejar de pulsar el registro EXTINT estaría vacío (boton no pulsado).
void comprobar_eint1(void){
    EXTINT =  EXTINT | 2;        //limpiamos flag de interrupcion
    if((eint1_nueva_pulsacion == 1) && ((EXTINT & 2) != 2) ){   // si el boton no esta pulsado reiniciamos, si esta pulsado no hacemos nada
        VICVectCntl2 = 0x20 | 15;                   
        VICIntEnable = VICIntEnable | 0x00008000;   // Activamos la interrupcion de eint1
        eint1_clear_nueva_pulsacion();
        cola_encolar_evento(CancelarAlarma, eint1_cuenta, 1);
    }
}

void comprobar_eint2(void){
    EXTINT =  EXTINT | 4;        //limpiamos flag de interrupcion
    if((eint2_nueva_pulsacion == 1) && ((EXTINT & 4) != 4) ){   // si el boton no esta pulsado reiniciamos, si esta pulsado no hacemos nada
        VICVectCntl3 = 0x20 | 16;                   
        VICIntEnable = VICIntEnable | 0x00010000;   // Activamos la interrupcion de eint2
        eint2_clear_nueva_pulsacion();
        cola_encolar_evento(CancelarAlarma, eint2_cuenta, 2);
    }
}
